...
Back in the Solution Requirements chapter (\ref{s:features}), we detailed some possible features that were interesting to be used in this work. In this section we will detail what extractors have been implemented.
...
Color extraction from images has been a long standing problem \todo{insert a bunch of references for color detection}. We wanted to extract the most perceptible colors from images so, in the parrot example above (\fig{parrot}), the system should associate the image with the colors red, green, white and black and avoid colors that have little relevance, like the small blue of  the feathers and also ignore small tonal variances in the reds and greens.
...
Although we didn't implement this plugin the way we desired, it demonstrates that color extraction is feasible and, with more time, a better method that does more than just averaging the colors, including detecting the most relevant ones, could be implemented. \todo{references needed}
