...
\section{Does organization by similarity assist image browsing?} % (fold)
\label{sub:Rodden}
\begin{figure}[ht]
	\centering
...
% \begin{figure}[ht]
% 	\centering
% 	\subfloat[Spot display.]{
% 		\includegraphics[width=0.5\columnwidth]{imgs-RelatedWork/Porta-spot}
% 		\label{fig:porta:a}
% 	}
% 	\subfloat[Shot display.]{
% 		\includegraphics[width=0.5\columnwidth]{imgs-RelatedWork/Porta-shot}
% 		\label{fig:porta:b}
% 	}
% 	\label{fig:porta}
% 	\caption{aaa}
...
\begin{figure}
	\centering					%%—-start of first subfigure—-
	\subfloat[Small Box with a Long Caption]{
		\label{fig:subfig:a}	%% label for first subfigure
		\includegraphics[width=0.4\columnwidth]{imgs-RelatedWork/Porta-spot}
	}
	\hspace{1in}
	...
	\subfloat[Big Box]{
		...
		\includegraphics[width=0.4\columnwidth]{imgs-RelatedWork/Porta-shot}
	}
...
Some of the various techniques were the simple image grid view, a grid view with variable and independent height and width (EIB), a view that animates images like they were shot from a distance and get closer to the user called Shot (\fig{porta:b}), a view where images quickly appear on random positions on screen named Spot (\fig{porta:a}), and some other less commons like one that simulates an cylinder created with images (Cylinder), and others less relevant (Rotor, Tornado and Tornado of Planes).
...
\section{An evaluation of color-spatial retrieval techniques for large image databases} % (fold)
\label{sub:Tan}
% não tem imagens
...
\section{Similarity pyramids for browsing and organization of large image databases} % (fold)
\label{sub:Chen}
